...
function connect(event) {
    ...
    if (username) {
        ...
        var socket = new SockJS('/courseWeb/chatroom');
...
function onConnected() {
    // 訂閱/topic/customerservice
    stompClient.subscribe('/topic/customerservice', onMessageReceived); // 當後端送訊息至/topic/customerservice時，會執行onMessageReceived()。
...
function onMessageReceived(payload) {
    ...
    //    message.content = '加入聊天室';
        message.content = message.sender + ' 加入聊天室';
    } else if (message.type === 'LEAVE') {
        messageElement.classList.add('event-message');
    //    message.content =  '離開聊天室';
